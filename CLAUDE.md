...
**Egress rate limiting requires runsc.toml ceilings:** When `network.egressRateLimit.rateBytesPerSecond` is set, the operator stamps `dev.gvisor.flag.qdisc{,-tbf-rate,-tbf-burst}` pod annotations. Old runsc releases reject them and the pod fails to start (the README documents the minimum release). The node's runsc.toml must carry nonzero `qdisc-tbf-rate`/`qdisc-tbf-burst` ceilings, a temporary requirement until a runsc validation fix lands upstream (see hack/setup.sh). The rate ceiling must stay >= the CRD Maximum on `rateBytesPerSecond` (1000000000000) or admission-valid sandboxes will fail at pod start. Burst is not part of the API. It is always derived in the operator (`network.EffectiveEgressBurstBytes`), never in the gateway or the CRD spec.
