...
**Egress rate limiting requires gVisor release-20260601.0+:** When `network.egressRateLimit` is set, the operator stamps `dev.gvisor.flag.qdisc{,-tbf-rate,-tbf-burst}` pod annotations; older runsc rejects them and the pod fails to start. Burst defaulting lives only in the operator (`network.EffectiveEgressBurstBytes`), never in the gateway or the CRD spec.
