...
After modifying api-gateway handlers or swaggo annotations:
```bash
make swagger  # Regenerates api/openapi/ from code annotations (swaggo)
```

The api-gateway uses code-first OpenAPI generation with swaggo. Add annotations to handlers
in `internal/api-gateway/handlers/` and run `make swagger`. CI runs `make check-swagger`
...
- `api/openapi/` - Generated OpenAPI docs for api-gateway (generated by swaggo from code annotations)
