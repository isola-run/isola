...
**Egress rate limiting requires gVisor release-20260601.0+ AND runsc.toml ceilings:** When `network.egressRateLimit` is set, the operator stamps `dev.gvisor.flag.qdisc{,-tbf-rate,-tbf-burst}` pod annotations; older runsc rejects them and the pod fails to start. The node's runsc.toml must carry nonzero `qdisc-tbf-rate`/`qdisc-tbf-burst` ceilings (runsc validates after each annotation in map order, so qdisc=tbf with runtime rate/burst of 0 fails intermittently). The ceilings must stay >= the CRD Maximum bounds on `EgressRateLimit` (1000000000000 / 4294967295) or admission-valid sandboxes will fail at pod start. Burst defaulting lives only in the operator (`network.EffectiveEgressBurstBytes`), never in the gateway or the CRD spec.
