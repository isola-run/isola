...
**Egress rate limiting requires runsc.toml ceilings:** When `network.egressRateLimitBytesPerSecond` is set, the operator stamps `dev.gvisor.flag.qdisc{,-tbf-rate,-tbf-burst}` pod annotations. Old runsc releases reject them and the pod fails to start (the README documents the minimum release). The node's runsc.toml must carry nonzero `qdisc-tbf-rate`/`qdisc-tbf-burst` ceilings, a temporary requirement until a runsc validation fix lands upstream (see hack/setup.sh). The rate ceiling must stay >= the CRD Maximum on `egressRateLimitBytesPerSecond` (1000000000000) or admission-valid sandboxes will fail at pod start. Burst is not part of the API. It is always derived in the operator (`network.EffectiveEgressBurstBytes`), never in the gateway or the CRD spec.

**Operator and sandbox-sidecar release together.** The operator's own `internalversion.Get().GitVersion` is stamped as the `isola.run/sidecar-version` annotation on sandbox pods at creation and mirrored into `Sandbox.Status.SidecarVersion`. The pod annotation is the durable source of truth; status is a re-derived mirror. Consumers (api-gateway) use it as a proxy for the sidecar capabilities shipped with the sandbox. Do not decouple these two components' release cycles — e.g. allowing `--sidecar-image` to reference a build different from the operator's own version would silently make this annotation a lie.
