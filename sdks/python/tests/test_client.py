...
def test_binary_io_body_rewound_on_transient_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    respx.post("http://localhost:8080/v1/sandboxes/s1/filesystem").mock(side_effect=capture_content)

    octet = {"Content-Type": "application/octet-stream"}
    with Isola(base_url="http://localhost:8080") as client:
        client._api.request(
            "POST", "/v1/sandboxes/s1/filesystem", content=body, headers=octet,
        )
...
def test_binary_io_body_rewound_on_connection_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    respx.post("http://localhost:8080/v1/sandboxes/s1/filesystem").mock(side_effect=handler)
