...
@respx.mock
def test_error_status_code_mapping(status_code: int, exc_type: type[APIError]) -> None:
...
@respx.mock
def test_unknown_status_code_raises_base_api_error() -> None:
    respx.get("http://localhost:8080/sandboxes/bad").mock(
...
def test_unexpected_exception_raises_api_connection_error() -> None:
    ...
    with Isola(base_url="http://localhost:8080") as client, patch.object(
        client._api._client, "request", side_effect=RuntimeError("unexpected")
