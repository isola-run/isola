...
export class Commands {
  ...
  async run(args: string[], opts: RunOptions = {}, req: RequestOptions = {}): Promise<CommandResult> {
    ...
    // waitTimeoutMs bounds the entire input+wait+read phase. Build one AbortSignal
    ...
    // run-phase deadline; pass it to the stdin writes and all three concurrent
    // waits.
    ...
    try {
      // stdin writes run inside the guarded block so a throw from writeStdin/
      // closeStdin (e.g. non-2xx) reaches the sibling-abort and user-signal /
      // run-deadline precedence handling below, and so the run-phase deadline
      // covers them.
